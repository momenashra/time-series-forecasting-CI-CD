...
def test_stationarity():
    ...
    assert (
        adfuller(stationary_series)[1] < 0.05
    ), "Stationary series detected as non-stationary!"
    assert (
        adfuller(non_stationary_series)[1] > 0.05
    ), "Non-stationary series detected as stationary!"


def test_split_sequences_multi_output():
    data = np.array([[i] for i in range(20)])  # Fake time series data
    X, y = split_sequences_multi_output(
        data, n_steps=3, forecast_horizon=2, n_features_out=1
    )
